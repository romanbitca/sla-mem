/**
 * The synthetic "Brightwave" workspace used by the demo seed (scripts/seed.ts) and the mock Slack
 * server (test/mock-slack): 15 people (two bots, one deactivated), channels, private channels,
 * DMs and group DMs, threads (one with ~40 replies), broadcasts, reactions with skin tones, edits,
 * deletions, mentions, unfurls, code, legacy-attachment and Block Kit bot messages, custom emoji,
 * and attachments (PNG, PDF, HTML, text, a video that never downloads, a hidden_by_limit stub).
 * Entirely fictional; deterministic for a given seed and day.
 */
import type {
  SlackAttachment,
  SlackBlock,
...
import { makePdf, makePng, PAINTERS, type Paint } from './media';
import { Rng } from './rng';
...
export const TEAM_ID = 'T0DEMO0001';
export const TEAM_NAME = 'Brightwave';
export const TEAM_DOMAIN = 'brightwave-demo';
export const DAY = 86_400;
/** Scales every conversation's activity; 1.6 gives ~9k messages over 200 days. Set per generation. */
let RATE_SCALE = 1.6;
...
export interface DemoFile extends DemoFileSpec {
  folder: string;
  bytes: Buffer | null;
}
...
export function slackUsers(): SlackUser[] {
  return PEOPLE.map((p) => ({
    id: p.id,
    team_id: TEAM_ID,
    name: p.name,
...
export function slackConversations(): SlackConversation[] {
...
export function customEmoji(): Record<string, string> {
...
export interface SyntheticWorkspace {
  ...
  /** Everything a later sync would see changed: edits, deletions, new reactions. */
  updates: Update[];
  customEmoji: Record<string, string>;
}

/** Messages per day at RATE_SCALE 1.6 is ~45; big targets also get a longer history. */
export function planScale(messages: number, days: number | null): { days: number; scale: number } {
  const d = days ?? (messages <= 20_000 ? 200 : Math.min(1500, Math.round(200 + messages / 400)));
  return { days: d, scale: Math.max(0.2, (1.6 * messages) / (45 * d)) };
}

/**
 * Generates the fictional "Brightwave" workspace: people, conversations, ~`messages` messages over
 * `days` days relative to now (so part of it is older than Slack Free's 90-day window), files and
 * a set of later edits/deletions.
 */
export function generateWorkspace(
  opts: { seed?: number; messages?: number; days?: number | null } = {},
): SyntheticWorkspace {
  const { days, scale } = planScale(opts.messages ?? 10_000, opts.days ?? null);
  RATE_SCALE = scale;
  const gen = newGen(opts.seed ?? 42, days);
  generateStories(gen);
  generateChatter(gen);
  const updates = generateUpdate(gen);
  return { gen, days, users: slackUsers(), conversations: slackConversations(), updates, customEmoji: customEmoji() };
}
